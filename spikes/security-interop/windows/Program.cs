...
static class Program
{
    ...
    static int RequestNoCertificate(string[] args)
    {
        ...
        using var handler = new HttpClientHandler(); handler.ServerCertificateCustomValidationCallback = (_, presented, _, _) => presented is not null && Convert.ToHexString(SHA256.HashData(ExportSpki(presented))) == expected;
        ...
        Console.WriteLine($"status={(int)response.StatusCode}"); return response.IsSuccessStatusCode ? 0 : 2;
    }
    ...
    static int Diagnostic(string[] args)
    {
        ...
        try { using var key = CngKey.Create(CngAlgorithm.ECDsaP256, keyName, new CngKeyCreationParameters { Provider = new CngProvider(ProviderName) }); result["persisted_minimal"] = "PASS"; using var ecdsa = new ECDsaCng(key); _ = ecdsa.SignData(new byte[] { 2 }, HashAlgorithmName.SHA256); result["persisted_signing"] = "PASS"; File.WriteAllText(Path.Combine(dir, "diagnostic-key.json"), JsonSerializer.Serialize(new { keyName, provider = ProviderName }, JsonOptions)); }
        catch (Exception ex) { result["persisted_minimal"] = Describe(ex); result["persisted_signing"] = "NOT-RUN"; }
        try { using var key = CngKey.Create(CngAlgorithm.ECDsaP256, $"{keyName}.export", new CngKeyCreationParameters { Provider = new CngProvider(ProviderName), ExportPolicy = CngExportPolicies.None }); try { _ = key.Export(CngKeyBlobFormat.EccPrivateBlob); result["non_exportable"] = "UNEXPECTED_SUCCESS"; } catch (Exception ex) { result["non_exportable"] = $"PASS:{Describe(ex)}"; } key.Delete(); } catch (Exception ex) { result["non_exportable"] = $"NOT-RUN:{Describe(ex)}"; }
    ...
    static string Describe(Exception ex) => $"{ex.GetType().FullName}; HRESULT=0x{ex.HResult:X8}; message={ex.Message}";
